...
你是一个专门制作“户晨风本人复刻 SFT 数据”的中文数据工程助手。

任务：
我会给你一条问答样本。它来自户晨风相关语料，但当前版本可能混入了总结稿、整理稿、旁白稿，
或者过于工整的问答包装。

你的任务不是把它修正得更理性，也不是改得更客观，而是：
尽量把它还原成“户晨风本人会怎么直接开口回答”的样子。

核心目标：
1. 第一优先级是“像户晨风本人原答”，不是“更正确”。
2. 保留原样本里的立场强度、判断力度、绝对化表达、情绪和收尾方式。
3. 不要替他说得更克制，也不要替他降火。
4. 不要编造原样本没有的新事实。
5. 如果原样本本身过于空泛、太像二手整理、无法可靠还原，就直接丢弃。

强制要求：
1. answer 必须像本人在直接接话，不要写成“用户问……”“这个问题……”“这涉及到……”。
2. answer 开头禁止出现：
   - 用户
   - 用户问
   - 用户提到
...
{{
  ...
  "rewritten_answer": "更贴近户晨风本人原答的版本",
  "style_score": 0.86,
  "rewrite_tags": ["direct_answer", "persona_faithful", "colloquial", "anti_summary"]
}}
...
async def rewrite_row(
    ...
    if not parsed:
        return {**row, "mid_drop_reason": "rewrite_invalid_json", "rewrite_status": "failed_invalid_json"}

    should_keep = bool(parsed.get("should_keep", True))
    if not should_keep:
        return {
            **row,
            "mid_drop_reason": str(parsed.get("drop_reason", "rewrite_llm_drop")).strip() or "rewrite_llm_drop",
    ...
    if not keep:
        return {
            **row,
            "question_mid": rewritten_question,
            "answer_mid": rewritten_answer,
            "style_score": style_score,
            "rewrite_tags": parsed.get("rewrite_tags", []),
            "rewrite_status": "rewritten_but_dropped",
            "mid_drop_reason": reason,
        }
...
async def process_rewrites(
    ...
    raw_log_path = output_dir / "mid_raw.txt"
    rewritten_progress_path = output_dir / "mid_rewritten_progress.jsonl"
    dropped_progress_path = output_dir / "mid_rewrite_dropped_progress.jsonl"
            ...
            try:
                ...
                failure = {
                    **row,
                    ...
                    "mid_drop_reason": f"request_failed:{type(exc).__name__}",
                }
                async with write_lock:
            ...
            recovered_dropped.append(
                {
                    **row,
                    "rewrite_status": "dropped_by_llm",
                    "mid_drop_reason": str(obj.get("drop_reason", "rewrite_llm_drop")).strip() or "rewrite_llm_drop",
                }
            )
            continue
...
def to_sharegpt(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "messages": [
            {"role": "user", "content": row["question_mid"]},
            {"role": "assistant", "content": row["answer_mid"]},
        ],
        "meta": {
            "date": row.get("date", ""),
            "bucket": row.get("bucket", row.get("meta", {}).get("bucket", "")),
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Build mid-style persona SFT dataset")
    ...
    parser.add_argument("--api_key_file", type=str, default=os.environ.get("OPENAI_API_KEY_FILE", ""))
    ...
    for row in rows:
        ...
        if keep:
            ...
            rule_dropped.append({**current, "mid_drop_reason": reason})

    if args.max_rows > 0:
        normalized_rows = normalized_rows[:args.max_rows]

    write_jsonl(output_dir / "mid_candidates.jsonl", normalized_rows)
    write_jsonl(output_dir / "mid_rule_dropped.jsonl", rule_dropped)

    recovered_rewritten, recovered_dropped, completed_uids = recover_from_raw_log(
        output_dir / "mid_raw.txt",
        normalized_rows,
    )
    ...
    preference_rows = [to_preference_pair(row) for row in rewritten_rows if row["answer_mid"].strip() != row["answer"].strip()]

    write_jsonl(output_dir / "mid_rewritten.jsonl", rewritten_rows)
    write_jsonl(output_dir / "mid_rewrite_dropped.jsonl", rewrite_dropped)
    write_jsonl(output_dir / "mid_clean_full.jsonl", rewritten_rows)
    write_jsonl(output_dir / "mid_train.jsonl", [to_sharegpt(r) for r in split_train])
    write_jsonl(output_dir / "mid_val.jsonl", [to_sharegpt(r) for r in split_val])
    write_jsonl(output_dir / "mid_test.jsonl", [to_sharegpt(r) for r in split_test])
    write_jsonl(output_dir / "mid_dpo_pairs.jsonl", preference_rows)
    ...
    report = {
        ...
        "model": args.model,
        "persona_faithful_rewrite": True,
    }
    with (output_dir / "mid_report.json").open("w", encoding="utf-8") as f:
    ...
    print(f"[INFO] report saved to: {output_dir / 'mid_report.json'}")
